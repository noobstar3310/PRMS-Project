...
<body>
        ...
        <div class="row mb-4 d-flex align-items-stretch">
            ...
            <div class="col-lg-4 col-md-6 mb-4">
                <div class="card bg-light text-dark shadow card-custom h-100">
                    ...
                    <div class="card-footer">
                        <form id="filterForm" action="pjr_sow_dashboard.php" method="post">
                            <div class="row mb-2">
                                <!-- Filter by BG -->
                                <div class="col-4">
                                ...
                                <!-- Filter by SOW No -->
                                <div class="col-4">
                                    <select class="form-control" name="SOW_No" id="sowNoSelect">
                                        <option value="">Select SOW No</option>
                                ...
                                <div class="col-4">
                                    <select class="form-control" name="Working_Group" id="workingGroupSelect">
                ...
                <div class="card bg-warning text-dark shadow card-custom h-100">
                    ...
                    <div class="card-footer">
                        ...
                        <form action="pjr_sow_dashboard.php" method="get" class="mt-2">
                            ...
                            <button type="submit" class="btn btn-dark btn-sm btn-custom">Show Empty
                                CAPEX/OPEX</button>
                        </form>
                        <form action="pjr_sow_dashboard.php" method="get" class="mt-2">
    ...
    $(document).ready(function() {
        $('#bgSelect').change(function() {
            ...
            $.ajax({
                ...
                success: function(data) {
                    $('#sowNoSelect').empty().append(
                        '<option value="">Select SOW No</option>');
                    $('#workingGroupSelect').empty().append(
                        '<option value="">Select Working Group</option>');

                    if (data.sowNos) {
                        $.each(data.sowNos, function(key, value) {
                            $('#sowNoSelect').append('<option value="' + value +
                                '">' + value + '</option>');
                        });
                    }

                    if (data.workingGroups) {
                        $.each(data.workingGroups, function(key, value) {
                            $('#workingGroupSelect').append('<option value="' +
                                value + '">' + value + '</option>');
                        });
                    }
                }
        ...
        // Trigger change event to load all options by default when no BG is selected
        $('#bgSelect').trigger('change');
    });

    function fetchFilteredOptions() {
        const bg = document.getElementById('bgSelect').value; // Get the selected BG

        // Make sure BG is selected
        if (bg) {
            // Use Fetch API to send request to the server
            fetch('fetch_options.php?bg=' + encodeURIComponent(bg))
                .then(response => response.json())
                .then(data => {
                    // Populate the SOW No dropdown
                    const sowSelect = document.getElementById('sowSelect');
                    sowSelect.innerHTML = '<option value="">Select SOW No</option>';
                    data.sowNumbers.forEach(sow => {
                        sowSelect.innerHTML += `<option value="${sow}">${sow}</option>`;
                    });

                    // Populate the Working Group dropdown
                    const workingGroupSelect = document.getElementById('workingGroupSelect');
                    workingGroupSelect.innerHTML = '<option value="">Select Working Group</option>';
                    data.workingGroups.forEach(group => {
                        workingGroupSelect.innerHTML += `<option value="${group}">${group}</option>`;
                    });
                })
